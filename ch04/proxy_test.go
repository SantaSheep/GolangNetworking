...
func proxy(from io.Reader, to io.Writer) error {
	...
	if toIsReader && fromIsWriter {
		// Send replicas since "froom" and "to" implement the necessary interfaces
...
func TestProxy(t *testing.T) {
	...
	// server listens for a ping message and responds with a pong message. All other messages are echoed back to the client
	...
	// proxySever proxies messages from client connections to the destination server
	// replies from the destination server are proxied back to the clients
